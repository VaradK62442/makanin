...
class MatrixSolution:
    ...
    def _add_to_existing_eqn(
        self, existing_eqn: str, addition_left: str, addition_right: str
    ) -> str:
        ...
        left += addition_left
        right += addition_right
        return f"{left} = {right}"

    def _count_variable_difference(self, equation: Equation) -> int:
        # count the number of variables in the equation
        return sum([1 for l in equation.U if l == VARIABLE]) - sum(
            [1 for l in equation.V if l == VARIABLE]
        )

    def _remove_invalid_equations(
        ...
        for eqn in equations:
            if (
                self._count_variable_difference(eqn)
                != 0  # want different num of variables
                and eqn.U != eqn.V
                and Equation(f"{eqn.V} = {eqn.U}") not in new_eqns
            ):
                new_eqns.append(eqn)
    ...
    def _get_equations(
        self, options: list[list[tuple[str, str]]], prefix_eqn=""
    ) -> list[Equation]:
        """
        ...
        for pair in options[0]:
            if prefix_eqn == "":
                new_eqn = f"{pair[0]} = {pair[1]}"
            else:
                new_eqn = self._add_to_existing_eqn(prefix_eqn, pair[0], pair[1])
            equations += self._get_equations(options[1:], new_eqn)

        return self._remove_invalid_equations(equations)

    def get_possible_equations(self) -> list[Equation]:
