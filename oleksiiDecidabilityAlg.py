...
def generate_all_equations(n):
    ...
    g = EquationGenerator(n, OleksiiSolver)
...
def get_desired_equations(equations):
    ...
    return [
        ...
        if get_num_letters_in_set(eqn.V, EquationSolver.LETTERS)
        == get_num_letters_in_set(eqn.W, EquationSolver.LETTERS)
    ]


def do_solns_exist(equations):
    def run(eqn) -> str | bool | None:
        ...
        def case1(eqn):
            # case 1: Cx... = xD...
            return (
                all(x in eqn.V for x in variables)
                and eqn.V[0] not in variables
                and eqn.W[0] in variables
                and eqn.W[1] in letters
            ) or (
                eqn.V[0] in variables
                and eqn.V[1] in letters
                and all(x in eqn.W for x in variables)
                and eqn.W[0] not in variables
            )
        ...
        def case2(eqn):
            ...
            return (
                all(x in eqn.V for x in variables)
                and eqn.V[0] not in variables
                and all(l in variables for l in eqn.W[:2])
            ) or (
                all(l in variables for l in eqn.V[:2])
                and all(x in eqn.W for x in variables)
                and eqn.W[0] not in variables
            )
        ...
        def prefixes_match(v, w):
            ...
            v_consts = v_consts[:min_length]
            w_consts = w_consts[:min_length]

            return v_consts == w_consts

        if not prefixes_match(eqn.V, eqn.W) or not prefixes_match(
            eqn.V[::-1], eqn.W[::-1]
        ):
            return False

        # check number of variables in eqn
        if get_num_letters_in_set(
            eqn.V, EquationSolver.VARIABLES
        ) != get_num_letters_in_set(eqn.W, EquationSolver.VARIABLES):
            return False
    ...
    print(f"Solutions found: {len([e for e in results if type(e) == str])}")
    ...
    for i, result in enumerate(results):
        ...
        elif result is False and valid_soln:
            print(f"Invalid result: {e}, {result}, {soln}")
            invalid_solns += 1
