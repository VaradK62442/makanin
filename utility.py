...
def analyse_reverse_eqns(n, filename):
    ...
    with (
        open(f"{filename}_true.txt", "w") as true_file,
        open(f"{filename}_false.txt", "w") as false_file,
    ):
        for e, (soln, valid_soln) in results.items():
            if valid_soln:
                true_file.write(f"{e}: x = {soln}\n")
                results_count[True] += 1
            else:
                false_file.write(f"{e}\n")
...
def find_soln_analysis(criteria, folder):
    ...
    for i in range(2, MAX + 1):
        count = find_solns_given_criteria(
            i, criteria, filename=f"{full_folder}/{folder}_{i}.txt"
        )
        print(f"n = {i}, count = {count}")
        with open(f"{full_folder}/{folder}.txt", "a") as file:
            print(f"n = {i}, count = {count}", file=file)
...
def soln_length_one(v, w):
    return soln_is_valid(v, w, "") and (
        soln_is_valid(v, w, "a") or soln_is_valid(v, w, "b")
    )
...
def soln_length_two(v, w):
    return (
        not soln_is_valid(v, w, "")
        and not soln_is_valid(v, w, "a")
        and not soln_is_valid(v, w, "b")
        and (
            soln_is_valid(v, w, "aa")
            or soln_is_valid(v, w, "ab")
            or soln_is_valid(v, w, "ba")
            or soln_is_valid(v, w, "bb")
        )
    )
...
def soln_one_and_two(v, w):
    return (
        not soln_is_valid(v, w, "")
        and (soln_is_valid(v, w, "a") or soln_is_valid(v, w, "b"))
        and (
            soln_is_valid(v, w, "aa")
            or soln_is_valid(v, w, "ab")
            or soln_is_valid(v, w, "ba")
